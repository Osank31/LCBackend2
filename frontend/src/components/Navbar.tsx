...
import { Terminal, Plus, LogOut, LogIn, UserPlus, ListCollapse } from 'lucide-react';
...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="bg-darkPanel border-b border-darkBorder h-14 flex items-center justify-between px-6 select-none">
      ...
      <Link to="/problems" className="flex items-center gap-2 text-white font-mono font-bold tracking-wider hover:text-brandBlue transition-colors duration-150">
        ...
        {user ? (
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 border border-darkBorder bg-darkBg px-3 py-1 font-mono text-xs">
              ...
              <span className="text-[#f3f4f6] font-semibold">{user.name.toUpperCase()}</span>
            </div>
