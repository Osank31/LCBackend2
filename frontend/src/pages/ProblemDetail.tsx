...
import { Code2, FileText, BookOpen, Terminal as TermIcon, Play, Send, Loader2, AlertTriangle } from 'lucide-react';
...
const ProblemDetail: React.FC = () => {
  ...
  const [consoleStatus, setConsoleStatus] = useState<'idle' | 'running' | 'success' | 'failed' | 'error'>('idle');
  ...
  const startFallbackPolling = (submissionId: string) => {
    setConsoleLogs((prev) => [...prev, '[SYSTEM] WEBSOCKET CONNECTION FAILED. FAILING OVER TO POLLING...']);
    let pollAttempts = 0;
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        const currentSubmission = statusRes.data?.data as SubmissionResponse;
  ...
  const handleSubmission = async (_: boolean) => {
    ...
    setConsoleLogs(['[SYSTEM] COMPILING CODE AND SPANNING SANDBOX CONTAINER...', '[SYSTEM] INJECTING SOLUTION CODE STUB...']);
    ...
    try {
      ...
      setConsoleLogs((prev) => [...prev, `[SYSTEM] GENERATED SUBMISSION ID: ${submissionId}`, '[SYSTEM] CONNECTING WEBSOCKET GATEWAY FOR REALTIME RESULTS...']);
      ...
      socket.on('connect', () => {
        setConsoleLogs((prev) => [...prev, '[SYSTEM] WEBSOCKET CONNECTED. LISTENING FOR VERDICT...']);
      });
  ...
  if (loading) {
    return (
      <div className="min-h-[calc(100vh-3.5rem)] flex flex-col items-center justify-center bg-darkBg text-text-secondary">
        <Loader2 size={40} className="animate-spin text-brandBlue mb-4" />
        <span className="font-mono text-xs uppercase tracking-widest">PROBING WORKSPACE DATASETS...</span>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-[calc(100vh-3.5rem)] flex items-center justify-center bg-darkBg p-6">
        <div className="max-w-md bg-darkPanel border border-brandRed text-brandRed p-6 font-mono text-xs">
          ...
          <p className="uppercase">{error || 'Unable to fetch problem statements.'}</p>
          <Link to="/problems" className="inline-block mt-4 text-brandBlue underline hover:text-white uppercase font-bold">
            ← RETREAT TO LIST
          </Link>
        </div>
      </div>
        ...
        <div className="flex-grow p-6 overflow-y-auto space-y-6">
          {activeTab === 'description' ? (
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="flex items-center gap-4 mb-2">
                  <h1 className="text-2xl font-mono font-bold text-white uppercase tracking-tight">{problem.title}</h1>
                  <span className={`px-2 py-0.5 text-[9px] font-bold border uppercase tracking-wider ${
                    problem.difficulty === 'Easy' ? 'text-brandGreen bg-brandGreen/10 border-brandGreen' :
                    problem.difficulty === 'Medium' ? 'text-brandAmber bg-brandAmber/10 border-brandAmber' :
                    'text-brandRed bg-brandRed/10 border-brandRed'
                  }`}>
                    {problem.difficulty}
                  </span>
                </div>
                <div className="text-[10px] font-mono text-text-disabled uppercase">ID: {problem._id}</div>
              </div>
              ...
              {problem.testCases && problem.testCases.length > 0 && (
                <div className="space-y-4 pt-4 border-t border-darkBorder font-mono">
                  <h3 className="text-xs font-bold text-text-secondary uppercase">SAMPLE_TESTCASES</h3>
                  
                  {problem.testCases.slice(0, 2).map((tc, idx) => (
                    <div key={idx} className="space-y-2 bg-darkBg border border-darkBorder p-4 text-xs">
                      <div>
                        <span className="font-bold text-brandBlue uppercase">Sample Input {idx + 1}:</span>
                        <pre className="mt-1 bg-darkPanel p-2 text-white overflow-x-auto select-all">{tc.input}</pre>
                      </div>
                      <div>
                        <span className="font-bold text-brandGreen uppercase">Sample Output {idx + 1}:</span>
                        <pre className="mt-1 bg-darkPanel p-2 text-white overflow-x-auto select-all">{tc.output}</pre>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-4 font-mono">
              <h2 className="text-base font-bold text-white uppercase border-b border-darkBorder pb-2">OFFICIAL_EDITORIAL</h2>
              {problem.editorial ? (
                <p className="text-sm leading-relaxed text-text-secondary whitespace-pre-wrap">{problem.editorial}</p>
              ) : (
                <div className="text-xs text-text-disabled uppercase py-10 text-center border border-dashed border-darkBorder">
                  NO OFFICIAL EDITORIAL SUBMITTED FOR THIS COMPILATION
                </div>
              )}
            </div>
          )}
      ...
      <div className="w-full md:w-1/2 flex flex-col bg-darkBg">
        ...
        <div className="h-12 bg-darkHeader border-b border-darkBorder flex items-center justify-between px-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-xs font-bold text-[#f3f4f6] uppercase">SOURCE_COMPILER</span>
          </div>

          {/* Language Selector */}
          <div className="flex items-center gap-2">
            <label className="font-mono text-[10px] text-text-secondary uppercase font-bold">LANGUAGE:</label>
            <select
              value={language}
        ...
        <div className="h-56 bg-darkPanel flex flex-col border-t border-darkBorder font-mono">
          ...
          <div className="h-10 bg-darkHeader border-b border-darkBorder flex items-center justify-between px-4 shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold text-[#f3f4f6] uppercase">INTERACTIVE_CONSOLE</span>
            </div>
          ...
          <div className="flex-grow p-4 overflow-y-auto text-xs space-y-1 bg-darkBg text-text-secondary select-all">
            {consoleStatus === 'idle' ? (
              <div className="text-text-disabled uppercase italic select-none">
                Console idle. Click Run or Submit to deploy solutions in sandbox container...
              </div>
            ) : (
              consoleLogs.map((log, index) => {
                if (log.startsWith('[TEST_CASE_ALERT_RED]')) {
                  const jsonStr = log.replace('[TEST_CASE_ALERT_RED]', '');
                  try {
                    const testCase = JSON.parse(jsonStr);
                    return (
                      <div key={index} className="my-3 p-4 bg-brandRed/10 border border-brandRed/30 rounded font-mono text-xs text-white space-y-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
                          <div className="bg-darkBg/60 p-2.5 rounded border border-darkBorder/40">
                            <span className="text-[9px] font-bold text-text-disabled uppercase block mb-1">INPUT:</span>
                            <pre className="text-white whitespace-pre-wrap select-all font-mono">{testCase.input}</pre>
                          </div>
                          <div className="bg-darkBg/60 p-2.5 rounded border border-darkBorder/40">
                            <span className="text-[9px] font-bold text-text-disabled uppercase block mb-1">EXPECTED OUTPUT:</span>
                            <pre className="text-brandGreen font-bold whitespace-pre-wrap select-all font-mono">{testCase.expectedOutput}</pre>
                          </div>
                        </div>
                        <div className="bg-darkBg/60 p-2.5 rounded border border-darkBorder/40">
                          <span className="text-[9px] font-bold text-text-disabled uppercase block mb-1">YOUR OUTPUT:</span>
                          <pre className="text-brandRed font-bold whitespace-pre-wrap select-all font-mono">{testCase.output || 'No output'}</pre>
                        </div>
                      </div>
                    );
                  } catch (e) {
                    return <div key={index} className="text-brandRed font-bold whitespace-pre-wrap font-mono">{log}</div>;
                  }
                }

                if (log.startsWith('[TEST_CASE_ALERT_BLUE]')) {
                  return (
                    <div key={index} className="my-3 p-4 bg-brandBlue/10 border border-brandBlue/30 rounded font-mono text-xs text-white space-y-2">
                      <div className="flex items-center gap-2 text-brandBlue font-bold uppercase tracking-wider text-xs">
                        <Play size={14} className="fill-brandBlue" />
                        <span>ALL TEST CASES PASSED ✓</span>
                      </div>
                      <p className="text-text-secondary text-xs font-mono">Your code executed and passed all provided test cases successfully.</p>
                    </div>
                  );
                }

                let textStyle = 'text-text-secondary';
                if (log.startsWith('[SYSTEM]')) textStyle = 'text-brandBlue font-bold';
                else if (log.startsWith('[STATUS]')) {
                  if (log.includes('ACCEPTED')) textStyle = 'text-brandGreen font-bold';
                  else if (log.includes('REJECTED')) textStyle = 'text-brandRed font-bold';
                  else textStyle = 'text-brandAmber font-bold';
                }
                else if (log.startsWith('[RESULT]') || log.startsWith('[INFO]')) textStyle = 'text-white font-bold';
                else if (log.startsWith('[CRITICAL_ERROR]')) textStyle = 'text-brandRed font-bold';
                
                return (
                  <div key={index} className={`${textStyle} whitespace-pre-wrap font-mono`}>
                    {log}
                  </div>
                );
              })
