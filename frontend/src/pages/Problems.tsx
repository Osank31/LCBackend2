...
const Problems: React.FC = () => {
		...
		try {
			...
			if (query.trim()) {
				response = await api.get(
					`/problems/search?query=${encodeURIComponent(query)}`
				);
			} else {
			...
			setError(
				err.response?.data?.message ||
					"Failed to fetch problem set. Please refresh."
			);
		} finally {
			setLoading(false);
		}
	...
	const handleDelete = async (id: string) => {
		if (
			!window.confirm(
				"Are you absolutely sure you want to delete this problem? This action is irreversible."
			)
		) {
			return;
		}
		...
		try {
			...
			alert(
				err.response?.data?.message ||
					"Unauthorized or failed to delete problem."
			);
		}
	};
		...
		<div className="min-h-[calc(100vh-3.5rem)] bg-darkBg p-8">
			<div className="max-w-6xl mx-auto space-y-6">
				...
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-darkBorder pb-6">
					<div>
						...
						<p className="text-xs text-text-secondary mt-1 font-mono uppercase">
							SOLVE CHALLENGES AND ACCUMULATE TECHNICAL PROWESS
						</p>
					</div>
				...
				<form
					...
					className="flex gap-2 bg-darkPanel border border-darkBorder p-3"
				>
				...
				{loading ? (
					<div className="flex flex-col items-center justify-center py-20 border border-darkBorder bg-darkPanel text-text-secondary">
						...
						<span className="font-mono text-xs uppercase tracking-widest">
							LOADING PROBLEMS FROM ARENA...
						</span>
					</div>
				) : problems.length === 0 ? (
					/* Empty State */
					<div className="text-center py-20 border border-dashed border-darkBorder bg-darkPanel/50">
						<span className="font-mono text-sm text-text-disabled uppercase">
							NO PROBLEMS DEPLOYED IN THIS VECTOR AT THIS TIME
						</span>
					</div>
				) : (
					/* Problems Data Table */
										...
										>
											...
											<td className="py-4 px-6 font-semibold text-white whitespace-nowrap">
												...
												>
													<Play
														size={10}
														className="text-brandBlue opacity-0 group-hover:opacity-100 transition-opacity duration-150"
													/>
													<span>
														{prob.title.toUpperCase()}
													</span>
												</Link>
											</td>
											...
											<td className="py-4 px-6 whitespace-nowrap">
												<span
													className={`inline-block px-2.5 py-0.5 text-[10px] font-bold border uppercase tracking-wider ${diffColor}`}
												>
													{prob.difficulty}
												</span>
											</td>
